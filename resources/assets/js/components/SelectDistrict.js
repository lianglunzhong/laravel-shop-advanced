...
Vue.component('select-district', {
	...
	methods: {
		setFromValue(value) {
			...
			const provinceId = _.findKey(this.provinces, province => province === value[0]);
			// 没找到，清空省的值
			if (!provinceId) {
				this.provinceId = '';
				return;
			...
			// 由于观察器的作用，这个时候城市列表已经变成了对应省的城市列表
      		...
      		const cityId = _.findKey(addressData[provinceId], city => city === value[1]);
      		// 没找到，清空城市的值
      		if (!cityId) {
        		this.cityId = '';
        		return;
      		...
      		const districtId = _.findKey(addressData[cityId], district => district === value[2]);
      		// 没找到，清空地区的值
      		if (!districtId) {
        		this.districtId = '';
        		return;
